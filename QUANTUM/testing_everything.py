...
def test_ep_round2(plaintext_bin, key_bin):
    from qiskit import QuantumCircuit, transpile
    from qiskit_aer import AerSimulator
    from oracle import KeyGenerator, apply_pbox, q_split, sbox1_gate, sbox2_gate
    ...
    # Round 2 — solo EP + XOR K2
    for i, r_idx in enumerate(EP_order):
        qc.cx(R[r_idx], ancillas[i])
    for i in range(8):
        qc.cx(k2_idx[i], ancillas[i])

    # Misura ancilla dopo EP+XOR K2
    ...
    print(f"[QUANTUM]  ancilla dopo EP+XOR K2 round 2: {measured}")
    print(f"[CLASSICO] EP(R)+XOR K2 round 2:           {xor2.to01()}")
    print(f"Match: {'✅' if measured == xor2.to01() else '❌ BUG IN EP O XOR K2 ROUND 2'}")

test_ep_round2('11001100', '0000111000')
